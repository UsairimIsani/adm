...
template <class T>
int Vector<T>::push(const T &item) {
...
int Vector<T>::insert(int index, const T &item) {
    ...
    if (size == capacity) resize(2 * capacity);
    int i;
    for (i = size - 1; i >= index; i--) arr[i + 1] = arr[i];
    arr[i + 1] = item;
    return ++size;
}

template <class T>
int Vector<T>::prepend(const T &item) {
...
template <class T>
T &Vector<T>::pop() {
    if (--size <= ceil(0.25 * capacity)) resize(0.5 * capacity);
    return arr[size];
}
...
int Vector<T>::remove(T item) {
    ...
    T *new_arr = new T[capacity];
    for (int i = 0; i < size; i++)
        if (arr[i] != item) new_arr[new_size++] = arr[i];
    size = new_size;
    delete[] arr;
    arr = new_arr;
    if (size <= ceil(0.25 * capacity)) resize(0.5 * capacity);
